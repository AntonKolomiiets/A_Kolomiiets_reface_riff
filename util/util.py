...
@st.cache_data # was just st.cahce
...
def save_audio_file(segment: pydub.AudioSegment, name: str, extension: str, output_dir: str) -> str:
    """
    Save the audio segment to a file and return the file path.
    """
    file_path = os.path.join(output_dir, f"{name}.{extension}")
    audio_bytes = io.BytesIO()
    segment.export(audio_bytes, format=extension)
